...
class Booking extends BaseService{
  ...
  public function insert($roomId, $userId, $checkInDate, $checkOutDate){
    ...
    // // convert date format - sql needs YY-MM-DD
    // $strtotime_in = explode("/",$checkInDate);
    // $strtotime_in = implode("/",array($strtotime_in[2], $strtotime_in[0], $strtotime_in[1]));
    //
    // $strtotime_out = explode("/",$checkOutDate);
    // $strtotime_out = implode("/",array($strtotime_out[2], $strtotime_out[0], $strtotime_out[1]));
